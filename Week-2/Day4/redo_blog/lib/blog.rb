...
class Blog
  attr_accessor :posts, :page, :number_page
    def initialize
        @posts = []
        @page = 1
        @number_page = 0
    ...
    def calc_number_page
        if @posts.length % 3 == 0
            @number_page = @posts.length / 3
        else
            @number_page = @posts.length / 3 + 1
        end
    end

    #sort articles
    def sort_posts_antichro(post_array)
         post_array.sort! { |x,y| y.date <=> x.date }
    end
    ...
    #     n = @page
    #    @posts[((n*3)-3)..(n*3-1)].each do |post|
    #          post.print
    #     end
    # end
